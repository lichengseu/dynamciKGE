...
def _calc(h, t, r):
    return torch.norm(h + r - t, p=1, dim=1).cpu().numpy().tolist()
...
def test_link_prediction(test_list):
    ...
    print('head(raw)\t\t\t' + str(l_mrr) + '\t\t\t' + str(l_mr) + '\t\t\t' + str(l_hit10) + '\t\t\t' + str(l_hit3) + '\t\t\t' + str(l_hit1))
    print('tail(raw)\t\t\t' + str(r_mrr) + '\t\t\t' + str(r_mr) + '\t\t\t' + str(r_hit10) + '\t\t\t' + str(r_hit3) + '\t\t\t' + str(r_hit1))
    print('average(raw)\t\t\t' + str((l_mrr + r_mrr) / 2) + '\t\t\t' + str((l_mr + r_mr) / 2) + '\t\t\t' + str((l_hit10 + r_hit10) / 2) + '\t\t\t' + str((l_hit3 + r_hit3) / 2) + '\t\t\t' + str((l_hit1 + r_hit1) / 2))
    print('head(filter)\t\t\t' + str(l_mrr_filter) + '\t\t\t' + str(l_mr_filter) + '\t\t\t' + str(l_hit10_filter) + '\t\t\t' + str(l_hit3_filter) + '\t\t\t' + str(l_hit1_filter))
    print('tail(filter)\t\t\t' + str(r_mrr_filter) + '\t\t\t' + str(r_mr_filter) + '\t\t\t' + str(r_hit10_filter) + '\t\t\t' + str(r_hit3_filter) + '\t\t\t' + str(r_hit1_filter))
    print('average(filter)\t\t\t' + str((l_mrr_filter + r_mrr_filter) / 2) + '\t\t\t' + str((l_mr_filter + r_mr_filter) / 2) + '\t\t\t' + str((l_hit10_filter + r_hit10_filter) / 2) + '\t\t\t' + str((l_hit3_filter + r_hit3_filter) / 2) + '\t\t\t' + str((l_hit1_filter + r_hit1_filter) / 2))
