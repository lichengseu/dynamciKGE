...
def _calc(h, t, r):
    return float(np.linalg.norm(np.array(h) + np.array(r) - np.array(t), ord=config.norm))
...
def test_link_prediction(test_list):
    ...
    #     l.append(int(linest[0]))
    #     l_f.append(int(linest[1]))
    #     r.append(int(linest[2]))
    #     r_f.append(int(linest[3]))
    ...
    for i, golden_triple in enumerate(test_list):
        ...
        # if i >= len(l):
        #     break
        # l_pos, l_filter_pos, r_pos, r_filter_pos = l[i], l_f[i], r[i], r_f[i]
    ...
    return (l_hit10_filter + r_hit10_filter) / 2
...
print("Waiting for preparing...")
while not os.path.exists(config.res_dir):
    time.sleep(5)
for epoch in range(config.train_times):
    if epoch % config.validation_step == 0:
        print("Waiting for validating epoch%s" % str(epoch))
        while not os.path.exists(config.res_dir + 'entity_parameters' + str(epoch)):
            time.sleep(60)

        time.sleep(60)
        entity2vec = json.loads(open(config.res_dir + 'entity_parameters' + str(epoch)).read())
        relation2vec = json.loads(open(config.res_dir + 'relation_parameters' + str(epoch)).read())

        print('Validation epoch%s starting...' % str(epoch))
        hit10 = test_link_prediction(config.valid_list)
        print('Validation epoch%s ending...' % str(epoch))
        ...
        if hit10 > best_hit10:
            ...
            bad_count += 1
        print("Best hit10:%s; Current hit10:%s; Bad count:%s" % (str(best_hit10), str(hit10), str(bad_count)))

        if bad_count >= bad_patience:
            is_early_stop = True
            print("Early stopped at epoch %s" % str(epoch))
            print("The best epoch is: %s" % str(best_epoch))
            print("The best hit10 is: %s" % str(best_hit10))
            with open(config.res_dir + "early_stop.txt", 'w') as wf:
                wf.write(str(best_epoch))
            break

if not is_early_stop:
    print("All train steps finished.")
    with open(config.res_dir + "early_stop.txt", 'w') as wf:
        wf.write(str(best_epoch))
